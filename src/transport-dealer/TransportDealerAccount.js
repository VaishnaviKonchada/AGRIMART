...
export default function TransportDealerAccount() {
  ...
  function getFullAddress() {
    const parts = [dealer.doorNo, dealer.location, dealer.mandal, dealer.district, dealer.state, dealer.pincode, dealer.country];
  ...
  return <div className="transport-dealer-account">
      {/* Header */}
      <div className="account-header">
        <div className="title-wrap">
          <h2>{t("🚚 Dealer Account")}</h2>
          <p className="subtitle">{t("Manage your profile and business details")}</p>
        </div>
        <button className="dashboard-btn" onClick={() => navigate("/transport-dealer-dashboard")}>
          <span className="dash-icon">📊</span>
          <span className="dash-label">{t("Dashboard")}</span>
        </button>
      </div>
      ...
      <div className="stats-grid">
        <div className="stat">
          ...
          <div className="content">
            <div className="label">{t("Total Deliveries")}</div>
        ...
        <div className="stat">
          <div className="icon">🚙</div>
          <div className="content">
            <div className="label">{t("Vehicles")}</div>
      ...
      <div className="quick-nav-buttons">
        <button className="nav-btn orders-btn" onClick={() => navigate("/transport-dealer/orders")}>
          <span className="nav-icon">📦</span>
          <span className="nav-label">{t("Confirmed Orders")}</span>
        </button>
        <button className="nav-btn payments-btn" onClick={() => navigate("/transport-dealer/payments")}>
          <span className="nav-icon">💰</span>
          <span className="nav-label">{t("Payment Details")}</span>
        </button>
        <button className="nav-btn vehicles-btn" onClick={() => {
        loadVehicles();
        setShowVehicleModal(true);
      }}>
          <span className="nav-icon">🚚</span>
          <span className="nav-label">{t("View Vehicles")}</span>
        </button>
        <button className="nav-btn" onClick={() => navigate("/transport-dealer/support")}>
          <span className="nav-icon">⚠️</span>
          <span className="nav-label">{t("Complaints")}</span>
        </button>
      </div>
      ...
      <div className="form-section">
        ...
        <fieldset className="form-grid" disabled={!editMode}>
          <div className="field">
            ...
            <input value={dealer.name} onChange={e => setDealer({
            ...dealer,
            name: e.target.value
          })} />
          </div>
          ...
          <div className="field">
            ...
            <input value={dealer.phone} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field">
            ...
            <input value={dealer.licenseNumber} onChange={e => setDealer({
            ...dealer,
            licenseNumber: e.target.value
          })} placeholder={t("DL1234567890")} />
          </div>
        </fieldset>
      </div>
      ...
      {/* Address Information */}
      <div className="form-section">
        <h3 className="section-title">{t("\uD83C\uDFE0 Address Information")}</h3>
        <fieldset className="form-grid" disabled={!editMode}>
          <div className="field">
            <label>{t("Country")}</label>
            <input value={dealer.country} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field">
            <label>{t("State")}</label>
            <input value={dealer.state} onChange={e => setDealer({
            ...dealer,
            state: e.target.value
          })} placeholder={t("Andhra Pradesh")} />
          </div>
          <div className="field">
            <label>{t("District")}</label>
            <input value={dealer.district} onChange={e => setDealer({
            ...dealer,
            district: e.target.value
          })} placeholder={t("Srikakulam")} />
          </div>
          <div className="field">
            <label>{t("Mandal")}</label>
            <input value={dealer.mandal} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field">
            ...
            <input value={dealer.pincode} onChange={e => setDealer({
            ...dealer,
            pincode: e.target.value
          })} placeholder="123456" />
          </div>
          <div className="field">
            <label>{t("Door No/House No")}</label>
            <input value={dealer.doorNo} onChange={e => setDealer({
            ...dealer,
            doorNo: e.target.value
          })} placeholder={t("E.g., 123-A")} />
          </div>
          <div className="field full">
            <label>{t("Full Location/Address")}</label>
            <input value={dealer.location} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field full">
            ...
            gap: '10px'
              ...
              fontSize: '15px'
                ...
                {dealer.coordinates && dealer.coordinates.lat && dealer.coordinates.lon ? `${dealer.coordinates.lat.toFixed(6)}, ${dealer.coordinates.lon.toFixed(6)}` : '--'}
              </span>
              {editMode && <button type="button" className="fetch-coords-btn" onClick={handleFetchCoordinates} disabled={fetchingCoords} style={{
              ...
              marginLeft: '4px'
                  ...
                  {fetchingCoords ? t('Fetching...') : t('Fetch from Address')}
                </button>}
              {coordsError && <span style={{
              color: 'red',
              marginLeft: 8,
              fontSize: '13px'
            }}>{coordsError}</span>}
      ...
      {showLocationModal && <div className="account-location-modal-overlay" onClick={() => setShowLocationModal(false)}>
          <div className="account-location-modal" onClick={e => e.stopPropagation()}>
            <div className="account-location-modal-header">
              <h3>{t("Pick Current Location")}</h3>
              <button type="button" className="account-location-modal-close" onClick={() => setShowLocationModal(false)}>
                ✕
              </button>
            </div>

            {locationPayload?.baseLocation && <p className="account-location-modal-text">{t("Detected:")}{locationPayload.baseLocation}</p>}

            {locationSuggestions.length > 0 && <div className="account-location-suggestions">
                {locationSuggestions.map((suggestion, idx) => <button key={`${suggestion}-${idx}`} type="button" className="account-suggestion-chip" onClick={() => handlePickSuggestedLocation(suggestion)}>
                    {suggestion}
                  </button>)}
              </div>}

            {famousNearby.length > 0 && <div className="account-famous-nearby">
                <span className="account-famous-title">{t("Nearby famous place(s):")}</span>
                <div className="account-famous-list">
                  {famousNearby.map((place, idx) => <button key={`${place}-${idx}`} type="button" className="account-famous-chip" onClick={() => handlePickFamousPlace(place)}>
                      {place}
                    </button>)}
                </div>
              </div>}
      ...
      {/* Banking Information */}
      <div className="form-section">
        <h3 className="section-title">{t("\uD83C\uDFE6 Banking Information")}</h3>
        <fieldset className="form-grid" disabled={!editMode}>
          <div className="field">
            <label>{t("Bank Name")}</label>
            <input value={dealer.bankName} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field">
            ...
            <input value={dealer.accountNumber} onChange={e => setDealer({
            ...dealer,
          ...
          <div className="field">
            ...
            <input value={dealer.ifscCode} onChange={e => setDealer({
            ...dealer,
            ifscCode: e.target.value
          })} placeholder={t("SBIN0001234")} />
          </div>
        </fieldset>
      </div>

      {/* Vehicle List Modal */}
      {showVehicleModal && <div className="vehicle-modal-overlay" onClick={() => setShowVehicleModal(false)}>
          <div className="vehicle-modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{t("\uD83D\uDE97 Your Vehicles (")}{vehicles.length})</h2>
              <button className="close-btn" onClick={() => setShowVehicleModal(false)}>✕</button>
            </div>
            
            {vehicles.length > 0 ? <div className="vehicle-list">
                {vehicles.map(vehicle => <div key={vehicle._id} className="vehicle-card-modal">
                    ...
                    <div className="vehicle-details">
                      ...
                      <div className="detail-row">
                        <span className="label">{t("Visible to Customers:")}</span>
                        <span className="value">{vehicle.isVisibleToCustomers ? 'Yes' : 'No'}</span>
                      </div>
                      {vehicle.pickupLocations?.length > 0 && <div className="detail-row">
                          <span className="label">{t("Pickup Locations:")}</span>
                          <div className="location-list">
                            {vehicle.pickupLocations.map((loc, i) => <span key={i} className="location-chip pickup">📍 {loc}</span>)}
                          </div>
                        </div>}
                      {vehicle.dropLocations?.length > 0 && <div className="detail-row">
                          <span className="label">{t("Drop Locations:")}</span>
                          <div className="location-list">
                            {vehicle.dropLocations.map((loc, i) => <span key={i} className="location-chip drop">🎯 {loc}</span>)}
                          </div>
                        </div>}
                    </div>
                  </div>)}
              </div> : <div className="no-vehicles">
                <div className="empty-icon">🚫</div>
                <h3>{t("No Vehicles Added")}</h3>
                <p>{t("You haven't added any vehicles yet.")}</p>
                <button className="add-vehicle-link-btn" onClick={() => {
            setShowVehicleModal(false);
            navigate("/transport-dealer/vehicles");
          }}>{t("\u2795 Add Your First Vehicle")}</button>
              </div>}
          </div>
        </div>}
      ...
      <div className="action-buttons">
        {!editMode ? <>
            <button className="save-btn" type="button" onClick={() => setEditMode(true)}>{t("\u270F\uFE0F Edit Details")}</button>
            <button className="logout-btn" type="button" onClick={handleLogout}>{t("\uD83D\uDEAA Logout")}</button>
          </> : <>
            <button className="save-btn" type="button" onClick={save} disabled={saving}>
              {saving ? t('⏳ Saving...') : t('💾 Save Changes')}
            </button>
            <button className="cancel-btn" type="button" onClick={() => setEditMode(false)} disabled={saving}>{t("Cancel")}</button>
            <button className="logout-btn" type="button" onClick={handleLogout}>{t("\uD83D\uDEAA Logout")}</button>
          </>}
      </div>
    </div>;
