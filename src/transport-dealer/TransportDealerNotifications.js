...
export default function TransportDealerNotifications() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const notifs = orders.map(order => ({
            ...
            type: "REQUEST",
            status: "Pending",
            customerName: order.customerId?.name || t("Customer"),
  ...
  const handleNotificationExpire = notifId => {
    const allNotifications = JSON.parse(localStorage.getItem("dealerNotifications") || "[]");
    const updated = allNotifications.map(n => n.id === notifId ? {
      ...n,
      status: "Expired",
  ...
  return <div className="transport-dealer-notifications">
      {/* Header */}
      ...
      <div className="notification-info-banner">
        ...
        <div className="banner-text">
          <p>{t("Accept or reject transport requests within")}<strong>{t("5 minutes")}</strong>{t(". Notifications that expire will be sent to other dealers.")}</p>
        </div>
      </div>
      ...
      <div className="filter-tabs">
        ...
        label: t("Requests"),
        type: "REQUEST",
        count: notifCounts.request
      }, {
        label: t("Messages"),
        type: "MESSAGE",
        count: notifCounts.message
      }, {
        label: t("Orders"),
        type: "ORDER",
        count: notifCounts.order
      }].map(({
        label,
        type,
        count
      ...
      <div className="notifications-list">
        {filteredNotifications.length > 0 ? filteredNotifications.map((notif, idx) => <div key={idx} className={`notification-card notification-${notif.type?.toLowerCase()}`}>
              {/* Timer Badge (only for REQUEST type) */}
              {notif.type === "REQUEST" && <div className={`timer-badge ${timers[notif.id] <= 60 ? "warning" : ""} ${timers[notif.id] === 0 ? "expired" : ""}`}>
                ...
                <div className="notif-title-section">
                  ...
                  <div className="notif-title">
                    ...
                    <p className="notif-time">
                      {notif.timestamp ? new Date(notif.timestamp).toLocaleString() : new Date().toLocaleString()}
                    </p>
                  </div>
                </div>
              ...
              <div className="notif-actions">
                {notif.type === "REQUEST" && <>
                    <button className="action-btn accept-btn" onClick={() => handleAcceptNotification(notif.id)} disabled={timers[notif.id] === 0}>{t("\u2705 Accept")}</button>
                    <button className="action-btn reject-btn" onClick={() => handleRejectNotification(notif.id)} disabled={timers[notif.id] === 0}>{t("\u2715 Reject")}</button>
                  </>}
                {notif.type === "MESSAGE" && <button className="action-btn view-btn" onClick={() => navigate("/transport-dealer/messages")}>{t("\uD83D\uDCAC View Chat")}</button>}
                {notif.type === "ORDER" && <button className="action-btn view-btn" onClick={() => navigate("/transport-dealer/orders")}>{t("\uD83D\uDCCB View Order")}</button>}
              </div>
            </div>) : <div className="empty-state">
            <div className="empty-icon">🔔</div>
            <p>{t("No notifications at the moment")}</p>
            <small>{t("You'll receive notifications when customers send requests")}</small>
          </div>}
      </div>
