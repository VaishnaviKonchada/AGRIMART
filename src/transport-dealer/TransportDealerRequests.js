...
export default function TransportDealerRequests() {
      ...
      try {
        if (action === "accept") {
          ...
          if (data?.success) {
            ...
            navigate("/transport-dealer/messages", { state: { chatId: data.chatId } });
            return;
          }
        }

        if (action === "reject") {
          const data = await apiPut(`transport-dealers/request/${requestId}/reject`, { reason: "Rejected via email action" });
          if (data?.success) {
            setEmailActionHandled(true);
            setRequests((prev) => prev.filter((r) => r._id !== requestId));
  ...
  const handleRejectRequest = async (orderId, reason) => {
    try {
      const data = await apiPut(`transport-dealers/request/${orderId}/reject`, { reason });
      if (data.success) {
        setRequests(requests.filter((r) => r._id !== orderId));
  ...
  return (
    <div className="transport-dealer-requests">
      {/* Header */}
      ...
      <div className="request-info-banner">
        ...
        <div className="info-text">
          <p>You have <strong>5 minutes</strong> to accept or reject each request. After 5 minutes, the request will be automatically rejected.</p>
        </div>
      </div>
      ...
      <div className="requests-container">
        {apiError && (
          <div className="empty-state">
            <p>Unable to load requests: {apiError}</p>
          </div>
        )}
        ...
        {loading ? (
            ...
            <div key={idx} className="request-card">
              ...
              <div
                className={`timer-badge ${
                  timers[request._id] <= 60 ? "warning" : ""
                } ${timers[request._id] === 0 ? "expired" : ""}`}
              >
              ...
              <div className="request-header">
                <div className="customer-info">
                  ...
                  <div className="customer-details">
                    <h3>{request.customerName || "Customer"}</h3>
                    <p className="customer-role">🚜 Farmer: {request.farmerName || "N/A"}</p>
                  </div>
                </div>
              ...
              <div className="request-details">
                <div className="meta-grid">
                  <div className="meta-item">
                    ...
                    <div>
                      <p className="label">Crop(s)</p>
                      {request.cropDetails && request.cropDetails.includes(',') ? (
                        <ul className="crop-list">
                          {request.cropDetails.split(',').map((crop, i) => (
                            <li key={i}>{crop.trim()}</li>
                          ))}
                        </ul>
                      ) : (
                        <p className="value">{request.cropDetails || request.cropItem || request.cropName || "Not specified"}</p>
                      )}
                    </div>
                  </div>
                  <div className="meta-item">
                  ...
                  <div className="meta-item">
                    ...
                    <div>
                      ...
                      <p className="value">{request.vehicleType || "Any"}</p>
                    </div>
                  </div>
                  <div className="meta-item">
                    <span className="icon">📏</span>
                    <div>
                      ...
                      <p className="value">{request.distance || "0"} km</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="route-card">
                  <div className="route-point">
                    ...
                    <div>
                      ...
                      <p className="value">{request.pickupLocation || "Not specified"}</p>
                    </div>
                  </div>
                  ...
                  <div className="route-point">
                    ...
                    <div>
                      ...
                      <p className="value">{request.dropLocation || "Not specified"}</p>
                    </div>
                  </div>
                </div>
                {request.fullAddress && request.fullAddress.trim().length > 0 && (
                  <div className="note-card">
                    <span className="icon">🏷️</span>
                    <p className="value"><b>Full Address:</b> {request.fullAddress}</p>
                  </div>
                )}
              ...
              <div className="request-actions">
                <button
                  ...
                  disabled={timers[request._id] === 0}
                >
                  ✅ Accept
                </button>
                <button
                  className="reject-btn"
                  onClick={() => handleRejectRequest(request._id, "Busy or unavailable")}
                  disabled={timers[request._id] === 0}
                >
                  ✕ Reject
                </button>
          ...
          <div className="empty-state">
            ...
            <small>New requests will appear here when customers book</small>
          </div>
        )}
      </div>
