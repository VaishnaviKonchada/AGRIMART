...
export default function TransportDealerReviews() {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        const reviewsData = orders.filter(order => order.feedback).map(order => ({
          id: order._id,
          orderId: order._id,
  ...
  const stats = useMemo(() => {
    ...
    const avgRating = total > 0 ? (dealerReviews.reduce((sum, r) => sum + (r.rating || 0), 0) / total).toFixed(1) : 0;
    const ratingCounts = {
  ...
  return <div className="transport-dealer-reviews">
      {/* Header */}
      ...
      <div className="rating-summary">
        <div className="rating-overview">
          <div className="overall-rating">
            ...
            <div className="rating-count">{t("Based on")}{stats.total}{t("reviews")}</div>
          </div>

          <div className="rating-breakdown">
            {[5, 4, 3, 2, 1].map(star => <div key={star} className="rating-row">
      ...
      <div className="filter-tabs">
        {["All", "5", "4", "3", "2", "1"].map(rating => <button key={rating} className={`filter-btn ${filterRating === rating ? "active" : ""}`} onClick={() => setFilterRating(rating)}>
            {rating === "All" ? t("All Reviews") : `${rating} ${rating === "1" ? t("1 Star") : t("Stars")}`}
          </button>)}
      </div>

      {/* Reviews List */}
      <div className="reviews-list">
        {filteredReviews.length > 0 ? filteredReviews.map((review, idx) => <div key={idx} className="review-card">
              {/* Reviewer Info */}
              <div className="review-header">
                <div className="reviewer-info">
                  ...
                  <div className="reviewer-details">
                    <h4>{review.customerName || t("Customer")}</h4>
              ...
              <div className="trip-reference">
                <span className="badge">{t("Trip #")}{review.tripId}</span>
              ...
              <div className="review-content">
                <h5>{t("Feedback")}</h5>
                <p>{review.comment || t("No comment provided")}</p>
              </div>

              {/* Review Highlights */}
              {review.highlights && review.highlights.length > 0 && <div className="review-highlights">
                  <h5>{t("Highlights")}</h5>
                  <div className="highlights-list">
                    {review.highlights.map((highlight, i) => <span key={i} className="highlight-tag">
                        ✓ {highlight}
                      </span>)}
                  </div>
                </div>}
              ...
              <div className="review-footer">
                {review.dealerResponse ? <div className="response">
                    <p className="response-label">{t("Your Response:")}</p>
                    <p className="response-text">{review.dealerResponse}</p>
                  </div> : <button className="respond-btn">{t("\uD83D\uDCAC Write Response")}</button>}
              </div>
            </div>) : <div className="empty-state">
            <div className="empty-icon">⭐</div>
            <p>{t("No reviews yet")}</p>
            <small>{t("Complete deliveries to receive reviews from customers")}</small>
          </div>}
      </div>
