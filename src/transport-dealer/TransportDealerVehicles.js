  ...
  try {
    ...
    if (snippet.startsWith("<!DOCTYPE") || snippet.toLowerCase().includes("<html")) {
      throw new Error(
        ...
        `Please confirm backend is running on ${API_BASE_URL} and you are logged in.`
      );
    }
...
export default function TransportDealerVehicles() {
  ...
  const isServiceAreaExpanded = (vehicleId, areaType) =>
    Boolean(expandedServiceAreas[`${vehicleId}:${areaType}`]);
    ...
    return (dealersList || [])
      .map((dealer) => ({
        ...
        vehicles: (dealer.vehicles || []).filter((vehicle) =>
          isQuantityMatch(vehicle.vehicleType, quantity)
        ),
      }))
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const url = editVehicleId
        ? `${API_BASE_URL}/dealer/vehicles/${editVehicleId}`
        : `${API_BASE_URL}/dealer/add-vehicle`;
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            ...
            `Failed to save vehicle (HTTP ${response.status})`
        );
      }
      if (!data.success) {
  ...
  const handleAddLocation = () => {
    ...
    if (newLocations.length > 0) {
      alert(
        `✅ Added ${newLocations.length} ${locationType} location(s)` +
          (duplicates.length > 0 ? ` (${duplicates.length} duplicate(s) skipped)` : "")
      );
    } else {
  ...
  const loadInlineDistricts = async (searchTerm = "") => {
    try {
      ...
      const url = searchTerm 
        ? `${API_BASE_URL}/locations/districts?search=${encodeURIComponent(searchTerm)}`
  ...
  const handleInlineAddLocation = () => {
    ...
    if (newLocations.length > 0) {
      alert(
        `✅ Added ${newLocations.length} ${inlineLocationType} location(s)` +
          (duplicates.length > 0 ? ` (${duplicates.length} duplicate(s) skipped)` : "")
      );
    } else {
      alert("All selected mandals already added");
  ...
  return (
    <div className="transport-dealer-vehicles">
      {/* Header */}
      ...
      >
        {showAddVehicle ? (editVehicleId ? "✕ Cancel Edit" : "✕ Cancel") : "+ Add Vehicle"}
      </button>

      <button
        className="add-vehicle-btn service-area-btn"
        onClick={() => navigate("/transport-dealer/service-area")}
        type="button"
      >
        📍 Service Areas
      </button>
      ...
      {/* Add Vehicle Form */}
      {showAddVehicle && (
        <div className="add-vehicle-form" ref={formRef}>
          <h3>{editVehicleId ? "✏️ Edit Vehicle" : "📝 Add New Vehicle"}</h3>
          ...
          <div className="form-grid">
            ...
            <div className="form-field">
              ...
              <input
                ...
                maxLength={10}
              />
              {newVehicle.insuranceExpiry && !isValidInsuranceInput(newVehicle.insuranceExpiry) && (
                <small style={{ color: "#c62828" }}>Use valid format: DD-MM-YYYY</small>
              )}
            </div>
            <div className="form-field">
              ...
              <input
                ...
                onChange={(e) => setNewVehicle({ ...newVehicle, quantity: Math.max(1, parseInt(e.target.value) || 1) })}
                min="1"
                placeholder="e.g., 1"
              />
            </div>
          ...
          <div className="form-checkbox">
            <input
              type="checkbox"
              checked={newVehicle.documentVerified}
              onChange={(e) => setNewVehicle({ ...newVehicle, documentVerified: e.target.checked })}
            />
            <label>I have verified all documents (RC, Insurance, Pollution)</label>
          </div>
      ...
      {showInlineServiceAreas && (
        <div className="add-vehicle-form service-areas-form">
          <h3>📍 Add Service Areas for Vehicle</h3>
          <p style={{color: '#666', marginBottom: '20px'}}>Define pickup and drop locations for this vehicle</p>
          ...
          {districtsLoadError && (
            <div style={{
              ...
              <p style={{margin: '0 0 10px 0'}}>⚠️ Failed to load districts from the server</p>
              <button
                onClick={() => loadInlineDistricts()}
                style={{
                  background: '#c00',
                  color: 'white',
                  border: 'none',
          ...
          {/* Loading state */}
          {!districtsLoadError && districts.length === 0 && (
            <div style={{
              background: '#f0f0f0',
              padding: '20px',
              borderRadius: '4px',
          ...
          {districts.length > 0 && (
            <>
              <div className="location-tabs" style={{display: 'flex', gap: '10px', marginBottom: '20px'}}>
                <button
                  className={`tab ${inlineLocationType === "pickup" ? "active" : ""}`}
                  onClick={() => setInlineLocationType("pickup")}
                  style={{
                    padding: '10px 20px',
                    border: 'none',
                    borderRadius: '4px',
                ...
                <button
                  className={`tab ${inlineLocationType === "drop" ? "active" : ""}`}
                  onClick={() => setInlineLocationType("drop")}
                  style={{
                    padding: '10px 20px',
                    border: 'none',
                    borderRadius: '4px',
              ...
              <div className="form-grid" style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px'}}>
                <div className="form-field">
                  ...
                  <input
                    ...
                    value={inlineDistrictSearch}
                    onChange={(e) => setInlineDistrictSearch(e.target.value)}
                    className="search-input"
                  />
                  <select
                    value={inlineSelectedDistrict}
                    onChange={(e) => {
                  ...
                  >
                    ...
                    {districts
                      .filter(d => d.district.toLowerCase().includes(inlineDistrictSearch.toLowerCase()))
                      .map(district => (
                ...
                <div className="form-field">
                  <label>Search Mandal:</label>
                  <input
                    type="text"
                    placeholder="🔍 Search mandal..."
                    value={inlineMandalSearch}
                    onChange={(e) => setInlineMandalSearch(e.target.value)}
                    className="search-input"
                    disabled={!inlineSelectedDistrict}
                  />
                  <select
                    value={inlineSelectedMandal}
                    onChange={(e) => {
                    ...
                    size="5"
                  >
                    <option value="" disabled>Select Mandal(s)</option>
                    {inlineMandals
                      .filter(m => m.name.toLowerCase().includes(inlineMandalSearch.toLowerCase()))
                      .map(mandal => (
              ...
              <button
                ...
                onClick={handleInlineAddLocation}
                disabled={inlineSelectedMandal.length === 0}
                style={{marginBottom: '20px'}}
              >
                ➕ Add {inlineLocationType === "pickup" ? "Pickup" : "Drop"} Location{inlineSelectedMandal.length > 1 ? "s" : ""}
                {inlineSelectedMandal.length > 0 && ` (${inlineSelectedMandal.length})`}
              </button>

              <div style={{marginBottom: '20px'}}>
                {inlinePickupLocations.length > 0 && (
                  <div style={{marginBottom: '15px'}}>
                    <h4>🔵 Pickup Locations ({inlinePickupLocations.length})</h4>
                    <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
                      {inlinePickupLocations.map(location => (
                        <div
                          key={location}
                          style={{
                            background: '#e3f2fd',
                        ...
                        >
                          <span>{location}</span>
                          <button
                            onClick={() => handleInlineRemoveLocation(location, "pickup")}
                            style={{
                              background: 'none',
                              border: 'none',
                              cursor: 'pointer',
                ...
                {inlineDropLocations.length > 0 && (
                  <div style={{marginBottom: '15px'}}>
                    <h4>🟡 Drop Locations ({inlineDropLocations.length})</h4>
                    <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
                      {inlineDropLocations.map(location => (
                        <div
                          key={location}
                          style={{
                            background: '#fff3e0',
                        ...
                        >
                          <span>{location}</span>
                          <button
                            onClick={() => handleInlineRemoveLocation(location, "drop")}
                            style={{
                              background: 'none',
                              border: 'none',
                              cursor: 'pointer',
                ...
                {inlinePickupLocations.length === 0 && inlineDropLocations.length === 0 && (
                  <p style={{color: '#999', fontStyle: 'italic'}}>No locations added yet</p>
                )}
              </div>
              ...
              <div style={{display: 'flex', gap: '10px'}}>
                <button
                  className="submit-btn"
                  onClick={saveInlineServiceAreas}
                  disabled={inlinePickupLocations.length === 0 || inlineDropLocations.length === 0}
                >
                  💾 Save Vehicle & Service Areas
                </button>
                <button
                  className="cancel-btn"
                  onClick={skipInlineServiceAreas}
                  style={{
                    background: '#ccc',
                    color: 'black',
                    padding: '10px 20px',
      ...
      <div className="vehicles-list">
        {vehicles.length > 0 ? (
            ...
            <div key={vehicle._id} className="vehicle-card">
              <div className="vehicle-header">
                ...
                <div className="vehicle-badges">
                  {vehicle.status === "Active" && <span className="status-badge active">🟢 Active</span>}
                  {vehicle.status === "Maintenance" && <span className="status-badge maintenance">🟡 Maintenance</span>}
                  {vehicle.status === "Inactive" && <span className="status-badge inactive">⚫ Inactive</span>}
                  {vehicle.documentVerified && (
              ...
              <div className="vehicle-info">
                ...
                <div className="info-grid">
                  ...
                  <div className="info-item">
                    <span className="icon">🛡️</span>
                    <div>
                      <span className="label">Insurance Valid</span>
              ...
              <div style={{
                ...
                <h5 style={{margin: '0 0 10px 0', fontSize: '14px', fontWeight: '600'}}>📍 Service Areas</h5>
                {!vehicle.isVisibleToCustomers && (
                  <p style={{ margin: '0 0 10px 0', fontSize: '12px', color: '#c62828' }}>
                    Hidden from customer page. Click "Show to Customers" to list this vehicle.
                  </p>
                )}
                  ...
                  <>
                    {vehicle.pickupLocations?.length > 0 && (
                      <div style={{marginBottom: '10px'}}>
                        <p style={{margin: '0 0 5px 0', fontSize: '12px', color: '#666'}}>🔵 Pickup ({vehicle.pickupLocations.length}):</p>
                        <div style={{display: 'flex', flexWrap: 'wrap', gap: '6px'}}>
                          {vehicle.pickupLocations
                            .slice(
                              0,
                              isServiceAreaExpanded(vehicle._id, "pickup")
                          ...
                          {vehicle.pickupLocations.length > 3 && (
                            <button
                              type="button"
                              onClick={() => toggleServiceAreaExpanded(vehicle._id, "pickup")}
                              style={{
                                background: '#e3f2fd',
                                padding: '4px 8px',
                                borderRadius: '12px',
                            ...
                            >
                              {isServiceAreaExpanded(vehicle._id, "pickup")
                                ? 'Show less'
                                : `+${vehicle.pickupLocations.length - 3} more`}
                            </button>
                          )}
                        </div>
                      </div>
                    ...
                    {vehicle.dropLocations?.length > 0 && (
                      <div>
                        <p style={{margin: '0 0 5px 0', fontSize: '12px', color: '#666'}}>🟡 Drop ({vehicle.dropLocations.length}):</p>
                        <div style={{display: 'flex', flexWrap: 'wrap', gap: '6px'}}>
                          {vehicle.dropLocations
                            .slice(
                              0,
                              isServiceAreaExpanded(vehicle._id, "drop")
                                ? vehicle.dropLocations.length
                                : 3
                            )
                            .map((location, idx) => (
                            <span key={idx} style={{
                          ...
                          {vehicle.dropLocations.length > 3 && (
                            <button
                              type="button"
                              onClick={() => toggleServiceAreaExpanded(vehicle._id, "drop")}
                              style={{
                                background: '#fff3e0',
                                padding: '4px 8px',
                                borderRadius: '12px',
                            ...
                            >
                              {isServiceAreaExpanded(vehicle._id, "drop")
                                ? 'Show less'
                                : `+${vehicle.dropLocations.length - 3} more`}
                            </button>
                          )}
                        </div>
                      </div>
              ...
              <div className="vehicle-actions">
                ...
                <button
                  ...
                  onClick={() => openServiceAreas(vehicle)}
                >
                  📍 Service Areas
                </button>
                <button
                  className={`action-btn ${vehicle.isVisibleToCustomers ? "visible-btn" : "hidden-btn"}`}
                  onClick={() => toggleVehicleVisibility(vehicle)}
                >
          ...
          <div className="empty-state">
            ...
            <small>Add your first vehicle to start deliveries</small>
          </div>
        )}
      </div>

      {showServiceModal && (
        <div className="vehicle-modal-overlay" onClick={() => setShowServiceModal(false)}>
          <div className="vehicle-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>📍 Set Service Areas</h3>
            ...
            <div className="locations-section">
              <div className="location-selector">
                ...
                <div className="location-input-group-parallel">
                  <div className="selector-wrapper">
                    <label className="selector-label">District</label>
                    <input
                      type="text"
                      placeholder="🔍 Search District..."
                  ...
                  <div className="selector-wrapper">
                    <label className="selector-label">Mandal</label>
                    <input
                      type="text"
                      placeholder="🔍 Search Mandal..."
                      value={mandalSearch}
                  ...
                  <button
                    ...
                    disabled={selectedMandal.length === 0}
                  >
              ...
              <div className="locations-display">
                {servicePickupLocations.length > 0 && (
                  <div className="location-group">
                    <h4>🔵 Pickup Locations ({servicePickupLocations.length})</h4>
                    <div className="location-chips">
                      {servicePickupLocations.map(location => (
                ...
                {serviceDropLocations.length > 0 && (
                  <div className="location-group">
                    <h4>🟡 Drop Locations ({serviceDropLocations.length})</h4>
                    <div className="location-chips">
                      {serviceDropLocations.map(location => (
            ...
            <div className="modal-actions">
              <button className="submit-btn" onClick={saveServiceAreas}>💾 Save Service Areas</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showMatchModal && (
        <div className="vehicle-modal-overlay" onClick={() => setShowMatchModal(false)}>
          <div className="vehicle-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>🔎 Check Customer Visibility</h3>
            ...
            <div className="locations-section">
              <div className="form-grid" style={{gridTemplateColumns: "1fr 1fr 1fr"}}>
                <div className="form-field">
                  <label>Customer Pickup (Mandal/District)</label>
                  <input
                    value={matchPickup}
                ...
                <div className="form-field">
                  <label>Customer Drop (Mandal/District)</label>
                  <input
                    value={matchDrop}
                    onChange={(e) => setMatchDrop(e.target.value)}
              ...
              <p style={{marginTop: "8px", color: "#64748b", fontSize: "12px"}}>
                Rules: Bike &le; 5kg, Auto &gt; 5kg to &le; 50kg, Truck &gt; 50kg to &le; 150kg.
              </p>

              {matchError && (
                <div style={{color: "#b91c1c", background: "#fee2e2", padding: "10px", borderRadius: "8px", marginTop: "10px"}}>
                  {matchError}
                </div>
              )}
              ...
              {matchResult && (
                <div style={{marginTop: "12px"}}>
                  {matchResult.matchedDealer && matchResult.matchedVehicles.length > 0 ? (
                    <div style={{background: "#dcfce7", color: "#166534", padding: "10px", borderRadius: "8px"}}>
                      ✅ Your dealer will be visible to this customer.
                      <div style={{marginTop: "8px", fontSize: "12px"}}>
                        Matching vehicles: {matchResult.matchedVehicles.map(v => v.vehicleName || v.vehicleType).join(", ")}
                      </div>
                    </div>
                  ) : (
                    <div style={{background: "#fff7ed", color: "#9a3412", padding: "10px", borderRadius: "8px"}}>
                      ❌ Your dealer will NOT be visible for this request.
                      <div style={{marginTop: "6px", fontSize: "12px"}}>
                        Check service areas, visibility toggle, and quantity rules.
                      </div>
                    </div>
                  )}
                </div>
